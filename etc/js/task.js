...
Array.prototype.myMap = function (
  /** @type {(item: unknown, i: number, array: unknown[]) => unknown} */ callback
) {
  const newArray = [];
...
function sliceArray(
  /** @type {unknown[]} */ anim,
  /** @type {number} */ beginSlice,
  /** @type {number} */ endSlice
) {
...
const squareList = (/** @type {number[]} */ arr) => {
  const positiveInts = arr.filter(
    (num) => Math.abs(num) === num && Math.floor(num) === Math.ceil(num)
  );
